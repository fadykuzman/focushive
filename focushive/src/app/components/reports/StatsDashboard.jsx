...
export default function StatsDashboard({ isOpen, onClose }) {
  ...
  if (loading) {
    return (
      <div id="stats-dashboard-overlay" className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={handleOverlayClick}>
        <div id="stats-dashboard-modal" className="bg-white rounded-lg p-6 w-[800px] max-w-90vw shadow-xl">
          <div className="text-center py-8">
            <div id="stats-loading-message" className="text-lg text-gray-600">Loading statistics...</div>
          </div>
        </div>
      </div>
    );
      ...
      <div id="stats-dashboard-overlay" className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={handleOverlayClick}>
        <div id="stats-dashboard-modal" className="bg-white rounded-lg p-6 w-[800px] max-w-90vw shadow-xl">
          <div className="flex justify-between items-center mb-4">
            <h2 id="stats-dashboard-title" className="text-2xl font-bold text-gray-800">Statistics</h2>
            <button id="stats-dashboard-close-btn" onClick={onClose} className="text-gray-500 hover:text-gray-700 text-2xl font-bold">×</button>
          </div>
          <div className="text-center py-8">
            <div id="stats-error-message" className="text-red-600 mb-4">{error}</div>
            <button 
              id="stats-retry-btn"
  ...
  return (
    <div id="stats-dashboard-overlay" className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={handleOverlayClick}>
      <div id="stats-dashboard-modal" className="bg-white rounded-lg p-6 w-[900px] max-w-95vw max-h-90vh overflow-y-auto shadow-xl">
        <div className="flex justify-between items-center mb-6">
          <h2 id="stats-dashboard-title" className="text-2xl font-bold text-gray-800">Productivity Statistics</h2>
        ...
        {!hasData ? (
          ...
          <div className="space-y-6">
            ...
            <div id="stats-quick-overview" className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div id="stats-today-focus-card" className="bg-blue-50 p-4 rounded-lg text-center">
                <div id="stats-today-focus" className="text-2xl font-bold text-blue-600">{formatDuration(daily.totalFocusTime)}</div>
                <div className="text-sm text-gray-600">Today's Focus</div>
              </div>
              <div id="stats-current-streak-card" className="bg-green-50 p-4 rounded-lg text-center">
                <div id="stats-current-streak" className="text-2xl font-bold text-green-600">{streaks.currentStreak}</div>
                <div className="text-sm text-gray-600">Current Streak</div>
              </div>
              <div id="stats-today-rate-card" className="bg-purple-50 p-4 rounded-lg text-center">
                <div id="stats-today-rate" className="text-2xl font-bold text-purple-600">{daily.completionRate}%</div>
                <div className="text-sm text-gray-600">Today's Rate</div>
              </div>
              <div id="stats-today-sessions-card" className="bg-orange-50 p-4 rounded-lg text-center">
                <div id="stats-today-sessions" className="text-2xl font-bold text-orange-600">{daily.totalSessions}</div>
                <div className="text-sm text-gray-600">Today's Sessions</div>
              </div>
            </div>

            {/* Streaks Section */}
            <div id="stats-streaks-section" className="bg-gray-50 p-4 rounded-lg">
              <h3 id="stats-streaks-title" className="text-lg font-semibold text-gray-800 mb-3">Streaks</h3>
              <div className="grid grid-cols-2 gap-4">
                <div id="stats-current-streak-details">
                  <div className="text-sm text-gray-600">Current Streak</div>
                  <div id="stats-current-streak-value" className="text-xl font-bold text-green-600">{streaks.currentStreak} days</div>
                </div>
                <div id="stats-longest-streak-details">
                  <div className="text-sm text-gray-600">Longest Streak</div>
                  <div id="stats-longest-streak-value" className="text-xl font-bold text-blue-600">{streaks.longestStreak} days</div>
                </div>
              </div>
            </div>

            {/* Weekly Overview */}
            <div id="stats-weekly-section" className="bg-gray-50 p-4 rounded-lg">
              <h3 id="stats-weekly-title" className="text-lg font-semibold text-gray-800 mb-3">This Week</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div id="stats-weekly-focus-time">
                  <div className="text-sm text-gray-600">Total Focus Time</div>
                  <div className="text-lg font-bold">{formatDuration(weekly.totalWeeklyFocusTime)}</div>
                </div>
                <div id="stats-weekly-sessions">
                  <div className="text-sm text-gray-600">Sessions</div>
                  <div className="text-lg font-bold">{weekly.totalWeeklySessions}</div>
                </div>
